...
var ENTITY_ATTR_MAP = (_ENTITY_ATTR_MAP = {}, _defineProperty(_ENTITY_ATTR_MAP, _draftJsUtils.ENTITY_TYPE.LINK, { url: 'href', rel: 'rel', target: 'target', title: 'title' }), _defineProperty(_ENTITY_ATTR_MAP, _draftJsUtils.ENTITY_TYPE.IMAGE, { src: 'src', alt: 'alt', 'data-original-url': 'href' }), _ENTITY_ATTR_MAP);
...
var OLD_INLINE_STYLES = OLD_COLORS.reduce(function (result, color, i) {
...
var decamelize = function decamelize(str, sep) {
...
function getTags(blockType) {
  switch (blockType) {
    ...
    case _draftJsUtils.BLOCK_TYPE.CHECKABLE_LIST_ITEM:
      return ['li'];
...
var MarkupGenerator = function () {
  function MarkupGenerator(contentState, checkedStateMap) {
    ...
    value: function renderBlockContent(block) {
      ...
      return entityPieces.map(function (_ref) {
        ...
        var content = stylePieces.map(function (_ref3) {
          ...
          var oldStyles = style.toArray().filter(function (style) {
            return Object.keys(OLD_INLINE_STYLES).indexOf(style) !== -1;
          });
          if (oldStyles.length > 0) {
            (function () {
              var styles = oldStyles.reduce(function (result, style) {
                return Object.keys(OLD_INLINE_STYLES[style]).reduce(function (r, prop) {
          ...
          if (blockType === _draftJsUtils.BLOCK_TYPE.CHECKABLE_LIST_ITEM) {
            var isChecked = _this.checkedStateMap[block.getKey()];
        ...
        if (entityType === _draftJsUtils.ENTITY_TYPE.LINK) {
          var attrs = dataToAttr(entityType, entity);
          var strAttrs = stringifyAttrs(attrs);
          return '<a' + strAttrs + '>' + content + '</a>';
        } else if (entityType === _draftJsUtils.ENTITY_TYPE.IMAGE) {
          var _attrs = dataToAttr(entityType, entity);
          var _strAttrs = stringifyAttrs(_attrs);
          return '<a href="' + _attrs.href + '"><img src="' + _attrs.src + '" alt="' + _attrs.alt + '" /></a>';
        } else {
          return content;
        }
      }).join('');
...
function stateToHTML(content, checkedStateMap) {
